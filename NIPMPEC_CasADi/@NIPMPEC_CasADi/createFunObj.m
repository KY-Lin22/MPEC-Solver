...
import casadi.*
%% symbolic variable
sigma = SX.sym('sigma', self.MPEC.Dim.sigma, 1); % dual variable for G
eta = SX.sym('eta', self.MPEC.Dim.eta, 1); % dual variable for C
gamma = SX.sym('gamma', self.MPEC.Dim.gamma, 1); % dual variable for PHI
...
for i = 1 : self.MPEC.Dim.sigma
    PSIg_formula(i, 1) = FB(sigma(i, 1), G(i, 1), self.MPEC.z);
    [PSIgSigma_diagVec_formula(i, 1), PSIgG_diagVec_formula(i, 1)] = FB_grad(sigma(i, 1), G(i, 1), self.MPEC.z);
end
FunObj.FB_G = Function('FB_G',...
    {sigma, G, self.MPEC.z}, {PSIg_formula},...
    {'sigma', 'G', 'z'}, {'PSIg'});
FunObj.FB_G_grad = Function('FB_G_grad',...
    {sigma, G, self.MPEC.z}, {PSIgSigma_diagVec_formula, PSIgG_diagVec_formula},...
...
for i = 1 : self.MPEC.Dim.gamma
    PSIphi_formula(i, 1) = FB(gamma(i, 1), PHI(i, 1), self.MPEC.z);
    [PSIphiGamma_diagVec_formula(i, 1), PSIphiPHI_diagVec_formula(i, 1)] = FB_grad(gamma(i, 1), PHI(i, 1), self.MPEC.z);
end
FunObj.FB_PHI = Function('FB_PHI',...
    {gamma, PHI, self.MPEC.z}, {PSIphi_formula},...
    {'gamma', 'PHI', 'z'}, {'PSIphi'});
FunObj.FB_PHI_grad = Function('FB_PHI_grad',...
    {gamma, PHI, self.MPEC.z}, {PSIphiGamma_diagVec_formula, PSIphiPHI_diagVec_formula},...
    {'gamma', 'PHI', 'z'}, {'PSIphiGamma_diagVec', 'PSIphiPHI_diagVec'});
            
%% Feasibility Restoration Phase
% FRP cost function
ZRef = SX.sym('ZRef', self.MPEC.Dim.Z, 1); % reference primal variable in FRP cost function
ZWeight = SX.sym('ZWeight', self.MPEC.Dim.Z, 1); % weight matrix in FRP cost function
Z = [self.MPEC.x; self.MPEC.p; self.MPEC.w];
FRP_L = 0.5 * (Z - ZRef)' * diag(ZWeight) * (Z - ZRef);

FunObj.FRP_L = Function('FRP_L',...
    {self.MPEC.x, self.MPEC.p, self.MPEC.w, ZRef, ZWeight},...
    {FRP_L},...
    {'x', 'p', 'w', 'ZRef', 'ZWeight'},...
    {'FRP_L'});

% Jacobian
FRP_Lx = jacobian(FRP_L, self.MPEC.x);
FRP_Lp = jacobian(FRP_L, self.MPEC.p);
FRP_Lw = jacobian(FRP_L, self.MPEC.w);
...
% Hessian
[FRP_L_hessian, ~] = hessian(FRP_L, [self.MPEC.x; self.MPEC.p; self.MPEC.w]);
